...
/* ============================================================================
   invent.js  –  wersja z poprawkami:  (1) Uwagi  (2) kod = Ewida
   ========================================================================== */
   (() => {
    "use strict";
    ...
    /* 1. Submit → Apps Script --------------------------------------------- */
    $(".form").on("submit", function (e) {
      e.preventDefault();
  
      codeInput.value = codeInput.value.toUpperCase();        // zawsze Ewida CAPS
  
      $.ajax({
        url: "https://script.google.com/macros/s/AKfycbxuFwFLAPK3pqNMBFddWeZrjwH0bC1rKnHttCeeZUoqC9jZM-KtP4MIOX9ONzfsQMotgg/exec",
        type: "POST",
        data: new FormData(this),
        processData: false,
        contentType: false,
    ...
    codeInput.addEventListener("keydown", evt => {
      ...
      if (row) {
        ...
        const displayRow = [...row];
        displayRow[0] = String(displayRow[0]).toUpperCase();   // Ewida CAPS
  
        window.lastSearchKey = displayRow[0];                  // dla historii
        codeInput.value      = displayRow[0];                  // (2) formularz wyśle Ewida
        ...
        /* ukryte pola (w tym Uwagi nawet, gdy puste) -------------------- */
        $(".lokalizacja")  .html(`<input name="Lokalizacja_gdzie_było" value="${row[3] || ""}">`);
        $(".numer_seryjny").html(`<input name="Numer_seryjny"          value="${row[2] || ""}">`);
        $(".dzial")        .html(`<input name="Dzial_gdzie_było"       value="${row[4] || ""}">`);
        $(".model")        .html(`<input name="Model"                  value="${row[1] || ""}">`);
        /* (1) Uwagi – nawet jeśli puste, nadpisujemy w „ewida” */
        $(".uwagi")        .html(`<input name="Uwagi"                  value="${row[5] || ""}">`);
